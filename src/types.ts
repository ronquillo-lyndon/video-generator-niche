...
export type SceneType = 'hook' | 'question' | 'answersIntro' | 'reveal' | 'explanation' | 'score' | 'cta';
export type RevealMode = 'after-each-question' | 'at-end';
...
export function buildScenes(questions: QuizQuestion[], timer: number, explanation = true, revealMode: RevealMode = 'after-each-question'): VideoScene[] {
  const rows: Omit<VideoScene, 'startTime'>[] = [{id: 'hook', type: 'hook', duration: 2}];
  if (revealMode === 'at-end') {
    questions.forEach(q => rows.push({id: `q-${q.id}`, type: 'question', questionId: q.id, duration: timer}));
    rows.push({id: 'answers-intro', type: 'answersIntro', duration: 2});
    questions.forEach(q => {
